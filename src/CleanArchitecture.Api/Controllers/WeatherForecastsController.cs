...
namespace CleanArchitecture.Api.Controllers
{
    ...
    public sealed class WeatherForecastsController : ControllerBase
    {
        ...
        {
            WeatherForecastDto forecast = await _mediator.Send(new GetWeatherForecastQuery(id));
            return Ok(forecast);
        }
        ...
        {
            List<WeatherForecastDto> forecasts = await _mediator.Send(new GetWeatherForecastsQuery(locationId));
            return Ok(forecasts);
        }
        ...
        {
            Guid id = await _mediator.Send(new CreateWeatherForecastCommand(forecast.TemperatureC, forecast.Date,
                forecast.Summary, forecast.LocationId));
